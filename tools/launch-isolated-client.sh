...
exec flock -n -E 75 "$TEST/runner.lock" bwrap --unshare-user --unshare-net --unshare-pid --die-with-parent \
  ...
  /bin/bash -c '
    ...
    world=$!
    # HOLOCRON_AUTH_LOGIN_REPLY selects the HISTORICAL/INVALID direct-D4 probe,
    # which answers RequestIDSignature (0xA609E6A7) with D4 and bypasses the
    # proven identification exchange. Prefer HOLOCRON_AUTH_ID_BOOTSTRAP.
    "$DOTNET_ROOT/dotnet" /home/dq/project-holocron/src/Holocron.Auth/bin/Debug/net8.0/Holocron.Auth.dll --test-key /opt/holocron-test/local-auth-key.pem ${HOLOCRON_AUTH_CAPTURE:+--capture-post-handshake} ${HOLOCRON_AUTH_LOGIN_REPLY:+--historical-invalid-direct-login-probe} ${HOLOCRON_AUTH_ID_BOOTSTRAP:+--probe-id-bootstrap} > /opt/holocron-test/logs/auth.log 2>&1 &
    auth=$!
